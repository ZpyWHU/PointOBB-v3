...
class RotatedSingleRoIExtractor(BaseRoIExtractor):
    ...
    def forward(self, feats, rois, roi_scale_factor=None):
        ...
        # expand_dims = (-1, self.out_channels * out_size * out_size) # 旧版本写法
        expand_dims = (-1, self.out_channels * out_size[0] * out_size[1])
        if torch.onnx.is_in_onnx_export():
            # Work around to export mask-rcnn to onnx
            roi_feats = rois[:, :1].clone().detach()
            roi_feats = roi_feats.expand(*expand_dims)
            # roi_feats = roi_feats.reshape(-1, self.out_channels, out_size, # 旧版本写法
            #                               out_size)
            roi_feats = roi_feats.reshape(-1, self.out_channels, *out_size) 
            roi_feats = roi_feats * 0
        else:
            roi_feats = feats[0].new_zeros(
                # rois.size(0), self.out_channels, out_size, out_size) # 旧版本写法
