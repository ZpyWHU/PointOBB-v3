...
class PointOBBHead(StandardRoIHead):
    ...
    def __init__(self, 
        ...
        self.agnostic_resize_classes2 = agnostic_resize_classes2
    ...
    def _init_layers(self):
        ...
        # from wholly wood, single layer feature map
        if self.use_ssff:
            ...
            if not use_dview:
                if self.use_angle_loss and stage == 0: 
                    ...
                    if self.add_angle_pred_begin and len(pos_inds) > 0:
                        ...
                        angle_add_proposals_list = []  # storage corresponding angle
                        start_idx = 0
    ...
    def _bbox_forward_train(self, x, proposal_list_base, proposals_list, proposals_valid_list, neg_proposal_list,
        ...
        # my: 增加对filtered box和filtered score的输出
        bbox_results.update(filtered_boxes=filtered_boxes)
        bbox_results.update(filtered_scores=filtered_scores)
    ...
    def _bbox_forward(self, x, rois, gt_true_bboxes, stage):
        ...
        # solution1: single layer feature map
        if self.use_ssff:
            x0 = x[0]
            ...
            bbox_feats = self.bbox_roi_extractor(
                single_layer_feature[:self.bbox_roi_extractor.num_inputs], rois)
        else:
            bbox_feats = self.bbox_roi_extractor(
    ...
    def _bbox_forward_test(self, x, rois, gt_true_bboxes, stage):
        ...
        if len(rois) > max_roi_num: # too many rois-OOM
            ...
            if self.use_ssff:
                ...
                single_layer_feature = (KV,)
                bbox_feats = self.bbox_roi_extractor(
                    single_layer_feature[:self.bbox_roi_extractor.num_inputs], rois_last)
            else:
                bbox_feats = self.bbox_roi_extractor(
