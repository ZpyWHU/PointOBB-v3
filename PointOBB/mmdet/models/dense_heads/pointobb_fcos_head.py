...
from mmdet.core.post_processing.bbox_nms_rotated  import multiclass_nms_rotated
...
class PointOBB_FCOS_Head(RotatedAnchorFreeHead):
    ...
    def loss(self,
        ...
        labels, bbox_targets, angle_targets, weights = self.get_targets(
            all_level_points, gt_bboxes, gt_labels, weights)
        ...
        if len(pos_inds) > 0:
            ...
            loss_bbox = self.loss_bbox(
                ...
                weight=(pos_centerness_targets*pos_weights),   # IAW
                avg_factor=centerness_denorm)
            if self.separate_angle:
                loss_angle = self.loss_angle(
                    pos_angle_preds, pos_angle_targets, weight=pos_weights.unsqueeze(0), avg_factor=num_pos)
            loss_centerness = self.loss_centerness(
                pos_centerness, pos_centerness_targets, weight=pos_weights, avg_factor=num_pos)   # IAW
        else:
    ...
    def get_targets(self, points, gt_bboxes_list, gt_labels_list, weights_list):
        ...
        labels_list, bbox_targets_list, angle_targets_list, weights_list = multi_apply(
        ...
        return (concat_lvl_labels, concat_lvl_bbox_targets,
                concat_lvl_angle_targets, concat_lvl_weights)

    def _get_target_single(self, gt_bboxes, gt_labels, weights, points, regress_ranges,
                           num_points_per_lvl):
        ...
        weights = weights[min_area_inds]
        labels[min_area == INF] = self.num_classes  # set as BG
        weights[min_area == INF] = mean_weight
        bbox_targets = bbox_targets[range(num_points), min_area_inds]
        angle_targets = gt_angle[range(num_points), min_area_inds]

        return labels, bbox_targets, angle_targets, weights
